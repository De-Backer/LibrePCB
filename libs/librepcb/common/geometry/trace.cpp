...
void TraceAnchor::serialize(SExpression& root) const {
  if (mNetPoint) {
    root.appendChild("junction", *mNetPoint, false);
  } else if (mVia) {
...
bool TraceAnchor::operator==(const TraceAnchor& rhs) const noexcept {
  return (mNetPoint == rhs.mNetPoint) && (mVia == rhs.mVia) &&
         (mPad == rhs.mPad);
}

TraceAnchor& TraceAnchor::operator=(const TraceAnchor& rhs) noexcept {
  mNetPoint = rhs.mNetPoint;
  mVia      = rhs.mVia;
  mPad      = rhs.mPad;
  return *this;
}

TraceAnchor TraceAnchor::netPoint(const Uuid& netpoint) noexcept {
  return TraceAnchor(netpoint, tl::nullopt, tl::nullopt);
}
