...
public:
  ...
  // Getters
  const tl::optional<Uuid>& tryGetNetPoint() const noexcept {
    return mNetPoint;
  }
  ...
  // Static Methods
  static TraceAnchor netPoint(const Uuid& netpoint) noexcept;
  static TraceAnchor via(const Uuid& via) noexcept;
  static TraceAnchor pad(const Uuid& device, const Uuid& pad) noexcept;

private:  // Methods
  TraceAnchor(const tl::optional<Uuid>& netpoint, const tl::optional<Uuid>& via,
              const tl::optional<PadAnchor>& pad) noexcept;

private:  // Data
  tl::optional<Uuid>      mNetPoint;
